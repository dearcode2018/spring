/**
 * 描述: 
 * ServiceLayerTransactionTest.java
 * 
 * @author qye.zheng
 *  version 1.0
 */
...
import com.hua.service.CustomService;
 ...
 *  */
// 使用事务注解 不能声明为final类型
public class ServiceLayerTransactionTest extends BaseTest {
	 ...
	 * 
	 * 注意: @Transactional  默认使用的是id为 transactionManager的
	 * 事务管理器，若id为其他的或多个事务管理器，则需要在参数中体现出来
	 * 
	 * 
	 */
	...
	public void testServiceLayer1() {
		try {
			...
			customService.insert1(entity);
			
			entity = new Custom();
			entity.setName("赵备小朋友02");
			entity.setAddress("广州市天河东路112号");
			entity.setBalance(10.34);
			entity.setStatus(CustomStatus.NORMAL);
			//customService.insert1(entity);
	...
	public void testServiceLayer2() {
		try {
			...
			entity.setBalance(10.34);
			entity.setStatus(CustomStatus.NORMAL);
			customService.insert2(entity);
			
			
		} catch (Exception e) {
			log.error("testServiceLayer2 =====> ", e);
		}
	}
	...
	public void testServiceLayer3() {
		try {
			...
			entity.setBalance(10.34);
			entity.setStatus(CustomStatus.NORMAL);
			customService.insert3(entity);
			
			
		} catch (Exception e) {
			log.error("testServiceLayer3 =====> ", e);
		}
	}
