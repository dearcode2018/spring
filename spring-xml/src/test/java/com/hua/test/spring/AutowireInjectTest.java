...
public final class AutowireInjectTest extends BaseTest {
	...
	public void testAutowire() {
		try {
			beanId = "AutowireBeanOne";
			beanId = "AutowireBeanTwo";
			beanId = "AutowireBeanThree";
			/*
			  根据bean 的 id 获取bean实例化对象，getBean(String name, Class<?>)
			  name 可以使bean id 或 name 检索值
			 */
			autowireBean = beanFactory.getBean(beanId, AutowireBean.class);
			System.out.println(autowireBean.getAdvanceBean());
			//System.out.println(autowireBean.toString());
			
			
		} catch (Exception e) {
			log.error("testAutowire =====> ", e);
		}
	}
